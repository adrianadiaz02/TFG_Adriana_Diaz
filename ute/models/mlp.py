...
class MLP(nn.Module):
    ...
    def __init__(self, opt, num_clusters):
        ...
        ### concat
        self.opt = opt
        self.fc1 = nn.Linear(opt.feature_dim, opt.embed_dim * 2) # >> Linear(64, 40x2)
    ...
    def forward(self, x):
        shape_dims = [*x.shape] # -> (batch_size, feature_dim)

        x = F.sigmoid(self.fc1(x)) # -> (batch_size, embed_dim * 2)
        x = F.sigmoid(self.fc2(x)) # -> (batch_size, embed_dim)
        embs = x.clone()
        
        proto_scores = self.prototype_layer(x) # -> (batch_size, num_clusters)
        
        if self.opt.time_loss:
          x = self.fc_last(x) # -> (batch_size, 1)
