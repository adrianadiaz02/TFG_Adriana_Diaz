...
class Accuracy(object):
    ...
    def mof_classes(self):
        ...
        for key, val in self._classes_MoF.items():
            ...
            ##########################
            mof_value = Accuracy.safe_divide(true_frames, all_frames)
            ##########################

            """logger.debug('label %d: %f  %d / %d' % (key, true_frames / all_frames,
                                                    true_frames, all_frames))"""
            
            logger.debug('label %d: %f  %d / %d' % (key, mof_value, true_frames, all_frames))
            
            #average_class_mof += true_frames / all_frames
            average_class_mof += mof_value
    ...
    def iou_classes(self):
        ...
        for key, val in self._classes_IoU.items():
            ...
            ######### AÑADIDO #########################
            iou_value = Accuracy.safe_divide(true_frames, union)
            ###########################################
            ...
            ########## AÑADIDO ###############################
            logger.debug('label %d: %f  %d / %d' % (key, iou_value, true_frames, union))
            #########################################
            
            if key not in self.exclude:
                #average_class_iou += true_frames / union
                average_class_iou += iou_value
            else:
                #excluded_iou += true_frames / union
                excluded_iou += iou_value
        ...
        ########## AÑADIDO ######################
        if len(self._classes_IoU) - len(self.exclude) > 0:
            average_iou_without_exc = average_class_iou / (len(self._classes_IoU) - len(self.exclude))
        else:
            average_iou_without_exc = 0

        average_iou_with_exc = (average_class_iou + excluded_iou) / len(self._classes_IoU) if len(self._classes_IoU) > 0 else 0
