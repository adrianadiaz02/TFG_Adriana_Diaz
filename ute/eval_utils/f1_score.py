...
class F1Score:
    ...
    def _pr2gt_convert(self):
        ...
        # Iterate over each GT label & its corresponding predicted labels list (for max method instead of hungarian matching)
        for gt_label, pr_label in self.gt2pr.items():

            ################ AÑADIDO #################
            if pr_label: # Check if the list of predicted labels is not empty
                if isinstance(pr_label, list) and len(pr_label) > 0:
                    # Ensure there is at least one predicted label and it's in a list format
                    m = np.sum(self.pr == pr_label[0]) # ya estaba
                    new_pr[self.pr == pr_label[0]] = gt_label # ya estaba
                else:
                    # Log an error or handle the case where pr_label is not a list or is an empty list
                    logger.error(f'Expected a list with at least one element for GT label {gt_label}, but got: {pr_label}')
            else:
                # Log an error or handle the case where pr_label is empty
                logger.error(f'No predicted labels found for GT label {gt_label}')
            ###########################################
